...
try
{
    ...
    // hard-coded instance for the site-wide total
    println '00000000-0000-0000-0000-000000000000##Site Total######citrixdaas.lifecyclestate=Active'
    ...
    mainResponse.json.value.each { group ->
        ...
        def instanceProperties = [
            'citrixdaas.sessionsupport' : sessionSupportMap.getOrDefault(group.SessionSupport, -1),
            'citrixdaas.deliverytype' : deliveryTypeMap.getOrDefault(group.DeliveryType, -1),
            'citrixdaas.desktopkind' : desktopKindMap.getOrDefault(group.DesktopKind, -1),
            'citrixdaas.lifecyclestate' : lifecycleStateMap.getOrDefault(group.LifecycleState, -1)
        ]
